...
async function extractStickersPerfect() {
  ...
  for (const c of crops) {
    ...
    alpha.fill(255); // Default: 100% opaque for all pixels!
    ...
    // Strict test for the outer tan background (#d29a39 / #d89c32)
    // Tan background has R: 185..230, G: 135..175, B: 30..80.
    // Bright yellow sticker border has G >= 185 and R >= 220!
    const isOuterTanBg = (r, g, b) => {
      if (g < 180 && b < 90 && r > 160 && r > g && g > b) {
        return true;
      }
      return false;
    };
    ...
    // ONLY start flood fill from the 4 outer image edges
    for (let x = 0; x < w; x++) {
      queue.push(x, 0);
      queue.push(x, h - 1);
    }
    ...
    while (head < queue.length) {
      ...
      if (isOuterTanBg(r, g, b)) {
        alpha[pos] = 0; // Only outer background becomes transparent!
    ...
    // Combine original RGB with perfect alpha channel
    ...
    console.log(`PERFECT extracted Nailong sticker (0 holes): ${outPath} (${w}x${h})`);
  }
}

extractStickersPerfect();
